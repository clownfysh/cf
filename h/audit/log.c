...
static h_core_bool_t enter_file(char *formatted_entry, FILE *file);
...
static h_core_bool_t write_entry_to_all_files(h_audit_log_t *log,
...
h_core_bool_t h_audit_log_add_file(h_audit_log_t *log, FILE *file)
{
  ...
  success = h_container_set_add(log->files, file);
...
h_audit_log_t *h_audit_log_create(FILE *file)
{
  ...
  if (so_far_so_good) {
    log->files = h_container_set_create(compare_files, H_CORE_NO_COPY_FUNCTION,
        H_CORE_NO_DESTROY_FUNCTION);
    if (log->files) {
      if (file) {
        if (h_container_set_add(log->files, file)) {
          so_far_so_good = h_core_bool_true;
        } else {
          so_far_so_good = h_core_bool_false;
        }
  ...
  if (!so_far_so_good && log) {
    if (log->files) {
      h_container_set_destroy(log->files);
    }
  }
...
h_core_bool_t h_audit_log_remove_file(h_audit_log_t *log, FILE *file)
{
  ...
  pthread_mutex_lock(&log->mutex);
  success = h_container_set_remove(log->files, file);
  pthread_mutex_unlock(&log->mutex);
...
h_core_bool_t enter_file(char *formatted_entry, FILE *file)
{
