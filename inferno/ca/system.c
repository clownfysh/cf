    ...
    unsigned long initial_time_step_count, cf_inferno_ca_systemey_t *systemey)
{
  ...
  if (system) {
    ...
    system->state_history = cf_x_case_array_create
      (initial_state_history_array_size, cf_inferno_ca_state_compare,
    ...
    if (system->state_history) {
      ...
      while ((state = cf_x_case_array_iterate_next(initial_state_history))) {
        ...
        state_copy = cf_inferno_ca_state_copy(state);
        cf_x_case_array_add(system->state_history, time_step, state_copy);
        time_step++;

        state_cell_count = cf_inferno_ca_state_get_cell_count(state);
    ...
    unsigned long time_step, unsigned long cell_index)
{
  ...
  cell = cf_inferno_ca_state_get_cell(state, cell_index);
...
void cf_inferno_ca_system_run(cf_inferno_ca_system_t *system, cf_x_core_run_t *run)
{
  ...
  cf_inferno_ca_state_t *current_state;
    ...
    char *filename)
{
  ...
  if (file) {
    cf_x_case_array_iterate_start(system->state_history);
    while ((state = cf_x_case_array_iterate_next(system->state_history))) {
      cell_count = cf_inferno_ca_state_get_cell_count(state);
      for (each_cell = 0; each_cell < cell_count; each_cell++) {
        cell_value = cf_inferno_ca_state_get_cell_value(state, each_cell);
    ...
    success = cf_x_core_bool_false;
    cf_x_core_trace("inferno_file_basic_create");
  }
...
{
  cf_inferno_ca_system_set_cell(system,
      system->current_time_step + relative_time_step, cell_index, cell);
}

cf_x_core_bool_t create_new_current_state(cf_inferno_ca_system_t *system)
{
  assert(system);
  cf_x_core_bool_t success;
  cf_inferno_ca_state_t *old_current_state;
  cf_inferno_ca_state_t *new_current_state;

  old_current_state = get_current_state(system);
  new_current_state = cf_inferno_ca_state_copy(old_current_state);
  if (new_current_state) {
    ...
    success = cf_x_core_bool_false;
    cf_x_core_trace("inferno_ca_state_copy");
  }

  return success;
}

cf_inferno_ca_state_t *get_current_state(cf_inferno_ca_system_t *system)
{
