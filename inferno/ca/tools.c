    ...
    cf_inferno_ca_select_initial_value_f select_initial_value)
{
  ...
  cf_inferno_ca_state_t *cell_state;
  cf_inferno_ca_t *cells;

  initial_state = cf_x_case_array_create(time_steps, cf_inferno_ca_state_compare,
      cf_inferno_ca_state_copy, cf_inferno_ca_state_destroy);
  if (initial_state) {
    cells = malloc(sizeof(cf_inferno_ca_t) * cell_count);
    if (cells) {
      for (each_time_step = 0; each_time_step < time_steps; each_time_step++) {
        ...
        cell_state = cf_inferno_ca_state_create_from_cells(cells, cell_count);
        if (cell_state) {
          cf_x_case_array_add(initial_state, each_time_step, cell_state);
        } else {
          cf_x_core_trace("inferno_ca_state_create_from_cells");
        }
      }
      free(cells);
    } else {
...
(cf_x_core_bitarray_t *bitarray)
{
  ...
  initial_state = cf_x_case_array_create(1, cf_inferno_ca_state_compare,
      cf_inferno_ca_state_copy, cf_inferno_ca_state_destroy);
  if (initial_state) {
    cells = malloc(sizeof(cf_inferno_ca_t) * cell_count);
    if (cells) {
...
{
  return cf_inferno_ca_create_initial_state(cell_count, time_steps,
      cf_inferno_ca_select_rule_0, cf_inferno_ca_select_value_salt_and_pepper);
}
...
(unsigned long cell_count, unsigned long time_steps)
{
  ...
  initial_state = cf_inferno_ca_create_initial_state(cell_count, time_steps,
      cf_inferno_ca_select_rule_0, cf_inferno_ca_select_value_0);
  if (initial_state) {
    single_cell_index = cell_count / 2;
    for (each_time_step = 0; each_time_step < time_steps; each_time_step++) {
      cell_state = cf_x_case_array_find(initial_state, each_time_step);
      if (cell_state) {
        cf_inferno_ca_state_set_cell_value(cell_state, single_cell_index, 1);
      } else {
        cf_x_core_trace("x_case_array_find");
      }
    }
  } else {
    cf_x_core_trace("inferno_ca_create_initial_state");
  }
...
(unsigned long cell_count)
{
  ...
  initial_state = cf_inferno_ca_create_initial_state(cell_count, 1,
      cf_inferno_ca_select_rule_0, cf_inferno_ca_select_value_0);
  if (initial_state) {
    single_cell_index = cell_count / 2;
    cell_state = cf_x_case_array_find_last(initial_state);
    cf_inferno_ca_state_set_cell_value(cell_state, single_cell_index, 7);
  } else {
    cf_x_core_trace("inferno_ca_create_initial_state");
  }
