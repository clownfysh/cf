...
{
  x_case_array_t *initial_state;
  unsigned long each_time_step;
  unsigned long each_cell;
  inferno_ca_state_t *cell_state;
  inferno_ca_t *cells;

  initial_state = x_case_array_create(time_steps, inferno_ca_state_compare,
  ...
  if (initial_state) {
    ...
    if (cells) {
      for (each_time_step = 0; each_time_step < time_steps; each_time_step++) {
        ...
        if (cell_state) {
          x_case_array_add(initial_state, each_time_step, cell_state);
        } else {
...
(x_core_bitarray_t *bitarray)
{
  ...
  initial_state = x_case_array_create(1, inferno_ca_state_compare,
      inferno_ca_state_copy, inferno_ca_state_destroy);
  if (initial_state) {
    cells = malloc(sizeof(inferno_ca_t) * cell_count);
    if (cells) {
      ...
      cell_state = inferno_ca_state_create_from_cells(cells, cell_count);
      if (cell_state) {
        x_case_array_add(initial_state, 0, cell_state);
      } else {
        x_core_trace("inferno_ca_state_create_from_cells");
      }
      free(cells);
...
(unsigned long cell_count, unsigned long time_steps)
{
  ...
  if (initial_state) {
    ...
    for (each_time_step = 0; each_time_step < time_steps; each_time_step++) {
      cell_state = x_case_array_find(initial_state, each_time_step);
      if (cell_state) {
        inferno_ca_state_set_cell_value(cell_state, single_cell_index, 1);
      } else {
        x_core_trace("x_case_array_find");
      }
    }
  } else {
...
(unsigned long cell_count)
{
  ...
  if (initial_state) {
    single_cell_index = cell_count / 2;
    cell_state = x_case_array_find_last(initial_state);
