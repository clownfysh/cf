...
static char display_color(cf_inferno_eos_actor_t *actor);

static cf_x_core_bool_t fitness_is_superior(double fitness_a, double fitness_b,
    cf_inferno_core_goal_t goal);

static double get_score(cf_inferno_eos_actor_t *actor);

static void meet_actor(cf_inferno_eos_actor_t *actor_a, cf_inferno_eos_actor_t *actor_b);

static void move_actor(cf_inferno_eos_actor_t *actor);

static void parse_display_gene(cf_inferno_eos_actor_t *actor,
    unsigned long gene_start_address, display_gene_t *display_gene);

static void parse_meet_gene(cf_inferno_eos_actor_t *actor,
    unsigned long gene_start_address, meet_gene_t *meet_gene);

static void parse_move_gene(cf_inferno_eos_actor_t *actor,
    unsigned long gene_start_address, cf_inferno_box_coordinate_t *move_gene);

static void parse_personality_gene(cf_inferno_eos_actor_t *actor,
    unsigned long gene_start_address, personality_gene_t *personality_gene);

char display_color(cf_inferno_eos_actor_t *actor)
{
...
double get_score(cf_inferno_eos_actor_t *actor)
{
  assert(actor);
  void *context;
...
int cf_inferno_eos_actor_compare_maximize(void *actor_a_void, void *actor_b_void)
{
  assert(actor_a_void);
  assert(actor_b_void);
  cf_inferno_eos_actor_t **actor_a;
  cf_inferno_eos_actor_t **actor_b;
  double actor_a_score;
  double actor_b_score;
  int compare_result;

  actor_a = (cf_inferno_eos_actor_t **) actor_a_void;
  actor_b = (cf_inferno_eos_actor_t **) actor_b_void;

  actor_a_score = get_score((cf_inferno_eos_actor_t *) *actor_a);
  actor_b_score = get_score((cf_inferno_eos_actor_t *) *actor_b);
...
void cf_inferno_eos_actor_init_actorey(cf_inferno_core_actorey_t *actorey)
{
  cf_inferno_core_actorey_init(actorey,
      ...
      cf_inferno_eos_actor_set_box_cell);
}
...
void meet_actor(cf_inferno_eos_actor_t *actor_a, cf_inferno_eos_actor_t *actor_b)
{
  ...
  box = cf_inferno_eos_system_get_box(actor_a->system);
  cf_inferno_box_system_swap(box, actor_a, actor_b);
}

void move_actor(cf_inferno_eos_actor_t *actor)
{
  cf_inferno_box_coordinate_t move_gene;
  ...
  cf_inferno_eos_actor_t *target_actor;
  double actor_score;
  ...
  box = cf_inferno_eos_system_get_box(actor->system);
  ...
  personality_gene_start_address = cf_inferno_core_get_gene_start_address
    (actor->solution, GENE_INDEX_PERSONALITY);
  parse_personality_gene(actor, personality_gene_start_address,
      &personality_gene);

  target_actor = cf_inferno_box_system_find_relative(box, actor, &move_gene);
  ...
  if (personality_gene.extrovert) {
    ...
    if ((!personality_gene.racist)
      ...
      goal = cf_inferno_eos_system_get_goal(actor->system);
    ...
    if (!target_actor) {
      cf_inferno_box_system_move_relative(box, actor, &coordinate);
    }
  }
}

void parse_display_gene(cf_inferno_eos_actor_t *actor,
    unsigned long gene_start_address, display_gene_t *display_gene)
{
...
void parse_meet_gene(cf_inferno_eos_actor_t *actor, unsigned long gene_start_address,
    meet_gene_t *meet_gene)
{
...
void parse_move_gene(cf_inferno_eos_actor_t *actor, unsigned long gene_start_address,
    cf_inferno_box_coordinate_t *move_gene)
{
  assert(actor);
  assert(move_gene);
...
void parse_personality_gene(cf_inferno_eos_actor_t *actor,
    unsigned long gene_start_address, personality_gene_t *personality_gene)
{
