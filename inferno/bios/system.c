    ...
    h_audit_log_t *log)
{
  ...
  if (system) {
    ...
    h_audit_log_trace(log, "bios", "malloc");
  }
    ...
    unsigned short max_solution_count)
{
  ...
  solutions = inferno_core_create_solutions_from_box(system->box,
      ...
      system->log);
  if (!solutions) {
    h_audit_log_trace(system->log, "bios", "create_solutions_from_box");
  }
...
{
  inferno_search_searchey_init(searchey, inferno_bios_system_create,
