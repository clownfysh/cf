#include "inferno/core/constants.h"
#include "inferno/search/tools.h"

static h_core_bool_t init_box_add_random_actors(inferno_box_system_t *box,
    inferno_box_coordinate_t *dimension_coordinate,
    inferno_core_create_actor_random_f create_actor_random, void *search_system,
    h_audit_log_t *log);

static h_core_bool_t init_box_add_actors_from_initial_solutions
(inferno_box_system_t *box, inferno_core_actorey_create_f actor_create,
    void *search_system, h_container_array_t *initial_solutions,
    h_audit_log_t *log);

h_container_array_t *inferno_core_create_solutions_from_box(inferno_box_system_t *box,
    unsigned long max_solution_count, inferno_core_get_solution_f get_solution,
    inferno_core_score_solution_f score_solution, inferno_core_goal_t goal,
    h_core_compare_f compare_maximize, h_core_compare_f compare_minimize,
    h_core_copy_f copy_object, void *context, h_audit_log_t *log)
{
  assert(box);
  assert(get_solution);
  assert(score_solution);
  assert(compare_maximize);
  assert(compare_minimize);
  assert(copy_object);
  assert(log);
  h_container_array_t *solutions;
  h_core_compare_f compare_objects;
  unsigned long box_volume;
  unsigned long index;
  h_core_bitarray_t *solution;
  h_core_bitarray_t *solution_copy;
  void *object;
  h_container_array_t *objects;

  if (INFERNO_CORE_GOAL_MAXIMIZE_SCORE == goal) {
    compare_objects = compare_maximize;
  } else {
    compare_objects = compare_minimize;
  }

  box_volume = inferno_box_system_get_volume(box);
  max_solution_count = h_core_min(max_solution_count, box_volume);

  objects = h_container_array_create(box_volume, compare_objects,
      copy_object, H_CORE_NO_DESTROY_FUNCTION);
  if (objects) {
    inferno_box_system_iterate_start(box);
    index = 0;
    while ((object = inferno_box_system_iterate_next(box))) {
      h_container_array_add(objects, index, object);
      index++;
    }

    h_container_array_sort(objects);

    solutions = h_container_array_create(max_solution_count,
        h_core_bitarray_compare, h_core_bitarray_copy,
        h_core_bitarray_destroy);
    if (solutions) {
      h_container_array_iterate_start(objects);
      for (index = 0; index < max_solution_count; index++) {
        object = h_container_array_iterate_next(objects);
        solution = get_solution(object);
        if (solution) {
          solution_copy = h_core_bitarray_copy(solution);
          if (solution_copy) {
            h_container_array_add(solutions, index, solution_copy);
          } else {
            h_audit_log_trace(log, "ih", "h_core_bitarray_copy");
          }
        } else {
          h_audit_log_trace(log, "ih", "h_core_bitarray_copy");
        }
        index++;
      }
    } else {
      h_audit_log_trace(log, "ih", "h_container_array_create");
    }
    h_container_array_destroy(objects);
  } else {
    solutions = NULL;
    h_audit_log_trace(log, "ih", "h_container_array_create");
  }

  return solutions;
}

inferno_box_system_t *inferno_core_create_actor_box(void *search_system,
    inferno_box_coordinate_t *dimension_coordinate,
    h_container_array_t *initial_solutions, inferno_core_actorey_t *actorey,
    h_audit_log_t *log)
{
  assert(search_system);
  assert(dimension_coordinate);
  assert(actorey);
  assert(log);
  inferno_box_system_t *box;

  box = inferno_box_system_create(dimension_coordinate, actorey->get_box_cell,
      actorey->set_box_cell, actorey->compare, actorey->copy, actorey->destroy,
      log);
  if (box) {
    if (init_box_add_random_actors(box, dimension_coordinate,
            actorey->create_random, search_system, log)) {
      if (initial_solutions) {
        if (!init_box_add_actors_from_initial_solutions(box, actorey->create,
                search_system, initial_solutions, log)) {
          inferno_box_system_destroy(box);
          box = NULL;
          h_audit_log_trace
            (log, "ih", "init_box_add_actors_from_initial_solutions");
        }
      }
    } else {
      inferno_box_system_destroy(box);
      box = NULL;
      h_audit_log_trace(log, "ih", "init_box_add_random_actors");
    }
  } else {
    h_audit_log_trace(log, "ih", "inferno_box_system_create");
  }

  return box;
}

h_core_bool_t init_box_add_random_actors(inferno_box_system_t *box,
    inferno_box_coordinate_t *dimension_coordinate,
    inferno_core_create_actor_random_f create_actor_random, void *search_system,
    h_audit_log_t *log)
{
  assert(box);
  assert(dimension_coordinate);
  assert(create_actor_random);
  assert(search_system);
  assert(log);
  h_core_bool_t success;
  inferno_box_coordinate_t coordinate;
  void *actor;

  success = h_core_bool_true;

  for (coordinate.x = 0;
       coordinate.x < dimension_coordinate->x;
       coordinate.x++) {
    for (coordinate.y = 0;
         coordinate.y < dimension_coordinate->y;
         coordinate.y++) {
      for (coordinate.z = 0;
           coordinate.z < dimension_coordinate->z;
           coordinate.z++) {
        actor = create_actor_random(search_system);
        if (actor) {
          inferno_box_system_add(box, &coordinate, actor);
        } else {
          success = h_core_bool_false;
          h_audit_log_trace(log, "cor3", "create_actor_random");
        }
      }
    }
  }

  return success;
}

/*  TODO: sort this array ourselves, by solution score  */
h_core_bool_t init_box_add_actors_from_initial_solutions(inferno_box_system_t *box,
    inferno_core_actorey_create_f actor_create, void *search_system,
    h_container_array_t *initial_solutions, h_audit_log_t *log)
{
  assert(box);
  assert(initial_solutions);
  assert(log);
  h_core_bool_t success;
  h_core_bitarray_t *bitarray;
  void *actor;

  success = h_core_bool_true;

  h_container_array_iterate_start(initial_solutions);
  while ((bitarray = h_container_array_iterate_next(initial_solutions))) {
    actor = actor_create(search_system, bitarray);
    if (actor) {
      inferno_box_system_replace_random(box, actor);
    } else {
      success = h_core_bool_false;
      h_audit_log_trace(log, "cor3", "actor_create");
    }
  }

  return success;
}

unsigned long inferno_core_get_gene_start_address(h_core_bitarray_t *solution,
    unsigned long gene_index)
{
  assert(solution);
  unsigned long address_of_gene_header;
  unsigned long start_address;

  address_of_gene_header = INFERNO_CORE_SOLUTION_BIT_COUNT_2N * gene_index;
  start_address = h_core_bitarray_get_unsigned_short(solution,
      address_of_gene_header) % INFERNO_CORE_SOLUTION_BIT_COUNT;

  return start_address;
}
