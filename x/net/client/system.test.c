...
#define CF_X_NET_CLIENT_SYSTEM_TEST_SLEEP_MICROSECONDS CF_X_CORE_STANDARD_SLEEP_MICROSECONDS
...
int main(int argc, char *argv[])
{
  ...
  client = cf_x_net_client_system_create("127.0.0.1", 20000, 20000,
      CF_X_NET_ENGINE_NO_GET_NAME_FUNCTION, &test_client_context, log);
  if (!client) {
    cf_x_core_trace_exit("x_net_client_create()");
  }
  cf_x_net_client_system_register_engine(client, CF_X_NET_ENGINE_TEST,
      CF_X_NET_TESTMESSAGE_TYPE_COUNT);
  cf_x_net_client_system_register_message_handler(client, CF_X_NET_ENGINE_TEST,
  ...
  do {
    ...
    usleep(CF_X_NET_CLIENT_SYSTEM_TEST_SLEEP_MICROSECONDS);
