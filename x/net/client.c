...
struct engine_container_t {
  ...
  cf_x_net_client_handle_message_f *message_handlers;
...
static cf_x_core_bool_t connect_to_server(cf_x_net_client_t *client);

static void destroy_engine_container(void *object);

static cf_x_core_bool_t ensure_client_is_connected(cf_x_net_client_t *client);

static cf_x_core_bool_t handle_disconnect(cf_x_net_client_t *client);

static cf_x_core_message_t *receive_message(cf_x_net_client_t *client);

cf_x_core_bool_t connect_to_server(cf_x_net_client_t *client)
{
  assert(client);
  ...
  for (port = client->server_min_port;
    ...
    client->socket
      = cf_x_net_clientsocket_create(client->server_ip_address, port);
    if (client->socket >= 0) {
      ...
      pthread_mutex_lock(&client->messaging_mutex);
      {
        ...
        if (client->post) {
          if (cf_x_net_exchange_register_post
            ...
            cf_x_net_clientsocket_destroy(client->socket);
...
cf_x_core_bool_t ensure_client_is_connected(cf_x_net_client_t *client)
{
  assert(client);
...
cf_x_core_bool_t handle_disconnect(cf_x_net_client_t *client)
{
  assert(client);
  cf_x_core_bool_t success;
...
void cf_x_net_client_process_messages(cf_x_net_client_t *client)
{
  ...
  cf_x_net_client_handle_message_f handler;
  engine_container_t *engine_container;
  cf_x_net_engine_id_t engine_id;
...
cf_x_core_message_t *receive_message(cf_x_net_client_t *client)
{
  cf_x_core_message_t *message;
