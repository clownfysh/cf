...
struct cf_x_net_starclient_t {
  ...
  cf_x_core_period_t *maintain_period;
...
static cf_x_net_client_t *find_client(cf_x_net_starclient_t *starclient,
    char *client_name);

static cf_x_net_client_t *get_random_client(cf_x_net_starclient_t *starclient);
...
(cf_x_net_starclient_t *starclient, cf_x_net_client_t *client);
    ...
    char *client_name)
{
  ...
  if (client) {
    if (cf_x_net_client_is_connected_to_server(client)) {
    ...
    unsigned short server_max_port, void *custom_client_context)
{
  ...
  client = cf_x_net_client_create(server_ip, server_min_port, server_max_port,
      CF_X_NET_ENGINE_NO_GET_NAME_FUNCTION, custom_client_context,
      starclient->log);
  if (client) {
    nameclient = cf_x_core_nameobject_create(client_name, client,
        CF_X_CORE_NO_COPY_FUNCTION, cf_x_net_client_destroy,
        cf_x_net_client_get_as_string);
    if (nameclient) {
...
cf_x_net_client_t *find_client(cf_x_net_starclient_t *starclient, char *client_name)
{
  assert(starclient);
  assert(client_name);
  cf_x_core_nameobject_t *nameclient_decoy;
...
cf_x_net_client_t *get_random_client(cf_x_net_starclient_t *starclient)
{
  assert(starclient);
  cf_x_core_nameobject_t *nameclient;
  cf_x_net_client_t *client;
...
(cf_x_net_starclient_t *starclient)
{
  ...
  starclient->maintain_period = cf_x_core_period_create(MAINTAIN_PERIOD);
...
void cf_x_net_starclient_create_rollback(cf_x_net_starclient_t *starclient)
{
  ...
  if (starclient->maintain_period) {
    cf_x_core_period_destroy(starclient->maintain_period);
  }
  if (starclient->unsent_messages) {
...
cf_x_net_client_t *cf_x_net_starclient_get_client(cf_x_net_starclient_t *starclient,
    int socket)
{
  assert(starclient);
  cf_x_core_nameobject_t *nameclient;
  ...
  while (!found_it && (nameclient = cf_x_case_list_iterate_next
    ...
    client = cf_x_core_nameobject_get_object(nameclient);
    each_socket = cf_x_net_client_get_socket(client);
...
void cf_x_net_starclient_process_messages(cf_x_net_starclient_t *starclient)
{
  ...
  if (cf_x_core_period_once(starclient->maintain_period)) {
    ...
    cf_x_net_engine_id_t engine_id, unsigned long message_type_count)
{
  ...
  *(starclient->message_type_counts + engine_id) = message_type_count;
  ...
  while ((nameclient = cf_x_case_set_iterate_next(starclient->clients))) {
    client = cf_x_core_nameobject_get_object(nameclient);
    if (!cf_x_net_client_register_engine(client, engine_id, message_type_count)) {
      success = cf_x_core_bool_false;
    }
  }
    ...
    cf_x_net_client_handle_message_f message_handler)
{
  ...
  while ((nameclient = cf_x_case_set_iterate_next(starclient->clients))) {
    client = cf_x_core_nameobject_get_object(nameclient);
    cf_x_net_client_register_message_handler
...
(cf_x_net_starclient_t *starclient, cf_x_core_message_t *message)
{
  assert(starclient);
  assert(message);
  cf_x_net_client_t *random_client;
  cf_x_core_bool_t success;
  cf_x_core_bool_t sent_to_client;

  random_client = get_random_client(starclient);
  if (random_client) {
    if (cf_x_net_client_send_message(random_client, message)) {
...
(cf_x_net_starclient_t *starclient, cf_x_core_message_t *message)
{
  ...
  while ((nameclient = cf_x_case_set_iterate_next(starclient->clients))) {
    ...
    if (message_copy) {
      if (!cf_x_net_client_send_message(client, message_copy)) {
        success = cf_x_core_bool_false;
    ...
    cf_x_net_client_t *client)
{
  ...
  for (engine_id = 0; engine_id < CF_X_NET_ENGINE_TYPE_COUNT; engine_id++) {
    ...
    if (-1 != message_type_count_long) {
      ...
      if (!cf_x_net_client_register_engine
    ...
    cf_x_net_client_t *client)
{
  ...
  while ((message_handler_info
    ...
    cf_x_net_client_register_message_handler(client,
