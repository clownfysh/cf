...
struct engine_container_t {
  ...
  cf_x_core_period_t *performance_period;
...
static running_engine_t *create_running_engine(cf_x_net_server_t *server,
    void *engine_object, cf_x_net_engine_id_t engine_id,
    cf_x_net_enginey_start_f start, cf_x_net_enginey_run_f run,
    cf_x_net_enginey_stop_f stop, unsigned short thread_index);
...
(engine_container_t *engine_container, cf_x_net_enginey_create_f create,
...
void close_disconnected_clients(cf_x_net_server_t *server)
{
  ...
  while ((client_post_object = cf_x_case_set_iterate_next(client_posts))) {
    if (server->postey->socket_closed(client_post_object)) {
      ...
      cf_x_net_clientsocket_destroy(client_socket);
    }
  }
...
(engine_container_t *engine_container, cf_x_audit_log_t *log)
{
  ...
  engine_container->performance_period
    = cf_x_core_period_create(PERFORMANCE_PERIOD_SECONDS);
...
void create_engine_container_rollback(engine_container_t *engine_container)
{
  ...
  if (engine_container->performance_period) {
    cf_x_core_period_destroy(engine_container->performance_period);
  }
    ...
    engine_container_t *engine_container)
{
  ...
  if (cf_x_core_period_once(engine_container->performance_period)) {
...
void *maintain_engine(void *maintaining_engine_object)
{
  ...
  cf_x_net_server_t *server;
  engine_container_t *engine_container;
  cf_x_core_period_t *maintenance_period;
  ...
  maintenance_period
    = cf_x_core_period_create(engine_container->maintain_schedule);
  if (maintenance_period) {
    while (!engine_container->stop_requested) {
      if (cf_x_core_period_once(maintenance_period)) {
        maintaining_engine->maintain(engine_object);
      }
      engine_sleep(server, engine_id);
    }
    cf_x_core_period_destroy(maintenance_period);
  } else {
