...
#define CF_X_NET_SERVER_SYSTEM_SLEEP_MICROSECONDS \
  CF_X_CORE_STANDARD_SLEEP_MICROSECONDS
...
(cf_x_net_server_system_t *server, void *engine_object,
    cf_x_net_engine_id_t engine_id,
    cf_x_net_engine_iengine_maintain_f maintain);

static cf_x_core_bool_t create_post_for_new_client
(cf_x_net_server_system_t *server, int client_socket);

static running_engine_t *create_running_engine
(cf_x_net_server_system_t *server, void *engine_object,
    cf_x_net_engine_id_t engine_id, cf_x_net_engine_iengine_start_f start,
    cf_x_net_engine_iengine_run_f run, cf_x_net_engine_iengine_stop_f stop,
    unsigned short thread_index);

static cf_x_core_bool_t create_thread(cf_x_net_server_system_t *server,
    pthread_t *pthread_address, cf_x_core_thread_f thread_function,
    void *thread);
...
(cf_x_net_server_system_t *server, engine_container_t *engine_container,
    unsigned short thread_index,
...
static void engine_sleep(cf_x_net_server_system_t *server,
    cf_x_net_engine_id_t engine_id);
...
static cf_x_core_bool_t cf_x_net_server_system_create_client_posts
(cf_x_net_server_system_t *server);

static cf_x_core_bool_t cf_x_net_server_system_create_engines
(cf_x_net_server_system_t *server);

static cf_x_core_bool_t cf_x_net_server_system_create_outbox
(cf_x_net_server_system_t *server);

static void cf_x_net_server_system_create_rollback
(cf_x_net_server_system_t *server);

static cf_x_core_bool_t cf_x_net_server_system_register_engine_create
(engine_container_t *engine_container, cf_x_net_engine_iengine_create_f create,
    cf_x_net_server_system_t *server, void *custom_server_object);

static cf_x_core_bool_t join_thread(cf_x_net_server_system_t *server,
    pthread_t thread);
...
static cf_x_core_bool_t serversocket_bind_listen
(cf_x_net_server_system_t *server);
...
static cf_x_core_bool_t start_engine_maintain_thread
(cf_x_net_server_system_t *server, cf_x_net_engine_id_t engine_id,
    engine_container_t *engine_container);

static cf_x_core_bool_t start_engine_run_thread
(cf_x_net_server_system_t *server, cf_x_net_engine_id_t engine_id,
    ...
    unsigned long message_type_count)
{
  ...
  if (so_far_so_good) {
    ...
    if (!engine_container->inbox) {
      cf_x_core_log_trace(server->log, "hnet", "cf_x_case_list_create");
      so_far_so_good = cf_x_core_bool_false;
    }
  }
...
(engine_container_t *engine_container, cf_x_core_log_t *log)
{
  ...
  if (engine_container->performance_period) {
    ...
    cf_x_core_log_trace(log, "hnet", "cf_x_sync_period_create");
    success = cf_x_core_bool_false;
  }
    ...
    cf_x_core_log_t *log)
{
  ...
  for (each_thread = 0; each_thread < MAX_THREADS_PER_ENGINE; each_thread++) {
    ...
    *(engine_container->thread_inboxes + each_thread)
    ...
    if (!*(engine_container->thread_inboxes + each_thread)) {
      success = cf_x_core_bool_false;
      cf_x_core_log_trace(log, "hnet", "cf_x_case_list_create");
      break;
    }
    ...
    int client_socket)
{
  ...
  if (post_object) {
    ...
    pthread_mutex_lock(&server->client_posts_mutex);
    {
      if (cf_x_case_set_add(server->client_posts, post_object)) {
        ...
        if (!cf_x_net_exchange_register_post
          ...
          cf_x_core_log_trace
            (server->log, "hnet", "cf_x_net_exchange_register_post");
          success = cf_x_core_bool_false;
        }
      } else {
        cf_x_core_log_trace(server->log, "hnet", "cf_x_case_set_add");
        success = cf_x_core_bool_false;
      }
    }
    ...
    cf_x_core_log_trace(server->log, "hnet", "create");
    success = cf_x_core_bool_false;
  }
...
running_engine_t *create_running_engine(cf_x_net_server_system_t *server,
    ...
    cf_x_net_engine_iengine_start_f start, cf_x_net_engine_iengine_run_f run,
    cf_x_net_engine_iengine_stop_f stop, unsigned short thread_index)
{
...
cf_x_core_bool_t create_thread(cf_x_net_server_system_t *server,
    pthread_t *pthread_address, cf_x_core_thread_f thread_function,
    void *thread)
{
  cf_x_core_bool_t success;
  ...
  if (!threads_maxed_out(server)) {
    ...
    cf_x_core_log_enter(server->log, "hnet", "threads_maxed_out");
  }
  ...
  if (cf_x_sync_period_once(engine_container->performance_period)) {
    ...
    if (messages_per_second_this_period > 0.0) {
      ...
      for (each_thread = 0; each_thread < engine_container->run_thread_count;
        ...
        if (!string) {
          cf_x_core_log_trace(server->log, "hnet", "cf_x_core_string_append");
        }
      }
    ...
    for (each_message = 0; each_message < target_number_of_messages_to_deliver;
      ...
      if ((message = cf_x_case_list_find_first(engine_container->inbox))) {
        if (cf_x_case_list_add_last(thread_inbox, message)) {
          ...
          cf_x_core_log_trace(server->log, "hnet", "cf_x_case_list_add_last");
        }
      }
    }
...
void engine_sleep(cf_x_net_server_system_t *server,
    cf_x_net_engine_id_t engine_id)
{
...
void *find_client_post(cf_x_net_server_system_t *server, int client_socket)
{
  ...
  if (decoy_post_object) {
    ...
    cf_x_core_log_trace(server->log, "hnet", "create_decoy");
...
cf_x_core_bool_t cf_x_net_server_system_create_client_exchange
(cf_x_net_server_system_t *server)
{
  assert(server);
  cf_x_core_bool_t success;

  server->client_exchange = cf_x_net_exchange_create(server->ipost);
  if (server->client_exchange) {
    success = cf_x_core_bool_true;
  } else {
    success = cf_x_core_bool_false;
    cf_x_core_log_trace(server->log, "hnet", "cf_x_net_exchange_create");
  }
...
(cf_x_net_server_system_t *server)
{
  ...
  cf_x_core_iobject_init(&server->client_posts_iobject,
      server->ipost->compare, CF_X_CORE_OBJECT_NO_COMPARE_EQUAL_F,
      CF_X_CORE_OBJECT_NO_COPY_F, server->ipost->destroy,
  ...
  if (server->client_posts) {
    ...
    success = cf_x_core_bool_false;
    cf_x_core_log_trace(server->log, "hnet", "cf_x_case_set_create");
  }
...
cf_x_core_bool_t cf_x_net_server_system_create_engines
(cf_x_net_server_system_t *server)
{
  assert(server);
  cf_x_core_bool_t success;
  ...
  if (server->engines) {
    for (each_engine_id = 0;
         each_engine_id < CF_X_NET_SERVER_SYSTEM_MAX_ENGINES;
         each_engine_id++) {
...
cf_x_core_bool_t cf_x_net_server_system_create_outbox
(cf_x_net_server_system_t *server)
{
  assert(server);
  cf_x_core_bool_t success;
...
cf_x_net_server_system_handle_message_f
cf_x_net_server_system_get_handler_for_message
...
cf_x_config_system_t *cf_x_net_server_system_get_config_system
(cf_x_net_server_system_t *server)
{
...
cf_x_core_bool_t cf_x_net_server_system_register_engine
(cf_x_net_server_system_t *server, cf_x_net_engine_id_t engine_id,
    void *custom_server_object, cf_x_net_engine_iengine_t *iengine,
    unsigned short min_run_threads, unsigned short max_run_threads,
    cf_x_net_maintain_t maintain_schedule, unsigned long message_type_count)
{
...
void cf_x_net_server_system_register_message_handler
(cf_x_net_server_system_t *server, cf_x_net_engine_id_t engine_id,
    unsigned long message_type,
...
cf_x_core_bool_t cf_x_net_server_system_send_message
(cf_x_net_server_system_t *server, void *message_object)
{
  cf_x_core_bool_t success;
...
cf_x_case_list_t *cf_x_net_server_system_take_unsent_messages
(cf_x_net_server_system_t *server)
{
  return NULL;
}

cf_x_core_bool_t join_thread(cf_x_net_server_system_t *server,
    pthread_t thread)
{
  cf_x_core_bool_t success;
...
void *maintain_engine(void *maintaining_engine_object)
{
  ...
  if (maintenance_period) {
    ...
    cf_x_core_log_trace(server->log, "hnet", "cf_x_sync_period_create");
  }
  ...
  while ((message_object = cf_x_case_list_iterate_next(server->outbox))) {
    ...
    if (client_post_object) {
      if (server->ipost->send_message(client_post_object, message_object)) {
        ...
        cf_x_core_log_trace(server->log, "hnet", "send_message");
      }
    } else {
      cf_x_case_list_iterate_remove(server->outbox);
      server->stats.discarded_message_for_nonexistent_client_count++;
...
cf_x_core_bool_t start_engine(cf_x_net_server_system_t *server,
    cf_x_net_engine_id_t engine_id)
{
  cf_x_core_bool_t success;
  unsigned short each_thread;
