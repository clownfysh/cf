...
int main(int argc, char *argv[])
{
  ...
  cf_x_net_engine_iengine_t hyperiengine;
  cf_x_core_imessage_t hypermessagey;
  cf_x_net_post_ipost_t hyperipost;
  ...
  cf_x_net_engine_iengine_init(&iengine, cf_x_net_testengine_create,
  ...
  server = cf_x_net_server_system_create("standard", port, port, TEST_SERVER_MAX_THREADS,
      &messagey, &ipost, CF_X_NET_ENGINE_NO_GET_NAME_FUNCTION,
      CF_X_NET_SERVER_SYSTEM_NO_CONFIG_SYSTEM, log);
  assert(server);

  if (!cf_x_net_server_system_register_engine(server, CF_X_NET_ENGINE_TEST,
          &custom_server_context, &iengine, 2, 2, CF_X_NET_MAINTAIN_CONSTANTLY,
  ...
  cf_x_net_engine_iengine_init(&hyperiengine, cf_x_net_testhyperengine_create,
  ...
  cf_x_net_post_ipost_init(&hyperipost, cf_x_net_http_post_compare,
  ...
  hyperserver = cf_x_net_server_system_create("http", hyperport, hyperport,
      TEST_HYPERSERVER_MAX_THREADS, &hypermessagey, &hyperipost,
