...
struct cf_x_xcache_system_t {
  cf_x_case_cache_t *container_cache;
  pthread_mutex_t mutex;
};

static void *find_copy_expire(cf_x_xcache_system_t *system,
    cf_x_core_uuid_t *object_uuid);

void *find_copy_expire(cf_x_xcache_system_t *system,
    cf_x_core_uuid_t *object_uuid)
{
  assert(system);
  assert(object_uuid);
  cf_x_xcache_item_t *item_decoy;
  cf_x_xcache_item_t *item;
  void *object_copy;

  object_copy = NULL;

  item_decoy = cf_x_xcache_item_create_decoy(object_uuid);
  if (item_decoy) {
    ...
    pthread_mutex_lock(&system->mutex);
    {
      ...
      if (item) {
        object_copy = cf_x_xcache_item_get_object_copy(item);
        if (!object_copy) {
          cf_x_core_trace("x_xcache_item_get_object_copy");
        }
        cf_x_xcache_item_expire(item);
      }
    }
    pthread_mutex_unlock(&system->mutex);

    cf_x_xcache_item_destroy_decoy(item_decoy);
  } else {
...
cf_x_core_bool_t cf_x_xcache_system_add(cf_x_xcache_system_t *system,
    ...
    unsigned long lifetime_seconds)
{
  ...
  item = cf_x_xcache_item_create(object_uuid, object, copy, destroy,
      lifetime_seconds);
  if (item) {
    success = cf_x_case_cache_add(system->container_cache, item);
    if (!success) {
      cf_x_xcache_item_destroy(item);
...
void cf_x_xcache_system_clear(cf_x_xcache_system_t *system)
{
  cf_x_case_cache_clear(system->container_cache);
}

cf_x_xcache_system_t *cf_x_xcache_system_create()
{
  cf_x_xcache_system_t *system;
  ...
  if (system) {
    system->container_cache = cf_x_case_cache_create(cf_x_xcache_item_compare,
        cf_x_xcache_item_copy, cf_x_xcache_item_destroy, cf_x_xcache_item_is_expired);
...
void cf_x_xcache_system_destroy(cf_x_xcache_system_t *system)
{
  assert(system);
  cf_x_case_cache_destroy(system->container_cache);
  if (0 != pthread_mutex_destroy(&system->mutex)) {
...
void *cf_x_xcache_system_find_wait_copy_expire
(cf_x_xcache_system_t *system, cf_x_core_uuid_t *object_uuid,
...
unsigned long cf_x_xcache_system_get_size(cf_x_xcache_system_t *system)
{
  return cf_x_case_cache_get_size(system->container_cache);
}

void cf_x_xcache_system_remove_objects(cf_x_xcache_system_t *system)
{
  pthread_mutex_lock(&system->mutex);
