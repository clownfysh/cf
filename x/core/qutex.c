...
{
  assert(qutex);
  cf_x_core_mutex_destroy(qutex->mutex);
  free(qutex);
}
...
{
  assert(qutex);
  while (!cf_x_core_qutex_trylock_exclusive(qutex)) {}
}

void cf_x_core_qutex_lock_shared(cf_x_core_qutex_t *qutex)
{
  assert(qutex);
  while (!cf_x_core_qutex_trylock_shared(qutex)) {}
}
...
cf_x_core_bool_t cf_x_core_qutex_trylock_exclusive(cf_x_core_qutex_t *qutex)
{
  ...
  cf_x_core_mutex_lock(qutex->mutex);
  {
  ...
  cf_x_core_mutex_unlock(qutex->mutex);
