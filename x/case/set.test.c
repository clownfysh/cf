...
int main(int argc, char *argv[])
{
  ...
  printf("creating set...\n");
  if (TEST_USING_LONGS) {
    x_core_unsigned_long_init_objectey(&unsigned_long_objectey);
    set = x_case_set_create(&unsigned_long_objectey);
  } else {
    x_core_unsigned_long_init_objectey(&uuid_objectey);
    set = x_case_set_create(&uuid_objectey);
  }
  assert(set);
  ...
  if (x_core_bool_false) {
    ...
    if (TEST_USING_LONGS) {
      while ((l = x_case_set_iterate_next(set))) {
        x_case_set_iterate_remove(set);
      }
    } else {
      while ((uuid = x_case_set_iterate_next(set))) {
        x_case_set_iterate_remove(set);
      }
    }
    printf("set size: %lu\n", x_case_set_get_size(set));
    assert(0 == x_case_set_get_size(set));
  }
