...
int main(int argc, char *argv[])
{
  ...
  if (TEST_USING_LONGS) {
    ...
    mbin = x_case_mbin_create(&objectey, X_CASE_MBIN_SET_TYPE_SET);
  } else {
  ...
  if (x_core_bool_false) {
    ...
    if (TEST_USING_LONGS) {
      while ((l = x_case_mbin_iterate_next(mbin))) {
        x_case_mbin_iterate_remove(mbin);
      }
    } else {
      while ((uuid = x_case_mbin_iterate_next(mbin))) {
        x_case_mbin_iterate_remove(mbin);
      }
    }
    printf("mbin size: %lu\n", x_case_mbin_get_size(mbin));
    assert(0 == x_case_mbin_get_size(mbin));
  }

  printf("clearing mbin...\n");
  x_case_mbin_clear(mbin);
  printf("mbin size: %lu\n", x_case_mbin_get_size(mbin));
  assert(0 == x_case_mbin_get_size(mbin));

  printf("destroying mbin...\n");
  x_case_mbin_destroy(mbin);
