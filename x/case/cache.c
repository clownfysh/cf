    ...
    void *object)
{
  ...
  success = x_case_set_add(cache->objects, object);
...
{
  x_core_qutex_lock_exclusive(cache->objects_qutex);
  x_case_set_clear(cache->objects);
  x_core_qutex_unlock_exclusive(cache->objects_qutex);
}
    ...
    x_core_condition_f remove_condition)
{
  ...
  if (cache) {
    ...
    cache->objects = x_case_set_create(&cache->objects_objectey);
    if (cache->objects) {
  ...
  if (!so_far_so_good) {
    if (cache->objects) {
      x_case_set_destroy(cache->objects);
    }
    if (cache->objects_qutex) {
...
void *x_case_cache_find(x_case_cache_t *cache,
    void *object)
{
  assert(cache);
  assert(object);
  ...
  found_object = x_case_set_find(cache->objects, object);
  x_core_qutex_unlock_shared(cache->objects_qutex);

  return found_object;
}

void *x_case_cache_find_copy(x_case_cache_t *cache,
    void *object)
{
  assert(cache);
  assert(object);
  ...
  x_core_qutex_lock_shared(cache->objects_qutex);
  object_copy = x_case_set_find_copy(cache->objects, object);
  x_core_qutex_unlock_shared(cache->objects_qutex);
...
unsigned long x_case_cache_get_size(x_case_cache_t *cache)
{
  ...
  x_core_qutex_lock_shared(cache->objects_qutex);
  size = x_case_set_get_size(cache->objects);
  x_core_qutex_unlock_shared(cache->objects_qutex);
...
void x_case_cache_remove_objects(x_case_cache_t *cache)
{
  ...
  x_core_qutex_lock_shared(cache->objects_qutex);
  {
    x_case_set_iterate_start(cache->objects);
    while ((object = x_case_set_iterate_next(cache->objects))) {
      if (cache->remove_condition(object)) {
        x_case_set_iterate_remove(cache->objects);
      }
    }
  }
  x_core_qutex_unlock_shared(cache->objects_qutex);
