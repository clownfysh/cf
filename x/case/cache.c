...
struct cf_x_case_cache_t {
  ...
  cf_x_core_qutex_t *objects_qutex;
    ...
    void *object)
{
  ...
  cf_x_core_qutex_lock_exclusive(cache->objects_qutex);
  success = cf_x_case_set_add(cache->objects, object);
  cf_x_core_qutex_unlock_exclusive(cache->objects_qutex);
    ...
    cf_x_core_condition_f remove_condition)
{
  ...
  if (so_far_so_good) {
    cache->objects_qutex = cf_x_core_qutex_create();
  ...
  if (!so_far_so_good) {
    ...
    if (cache->objects_qutex) {
      cf_x_core_qutex_destroy(cache->objects_qutex);
    }
    free(cache);
    cache = NULL;
  }
    ...
    void *object)
{
  ...
  cf_x_core_qutex_lock_shared(cache->objects_qutex);
  found_object = cf_x_case_set_find(cache->objects, object);
  cf_x_core_qutex_unlock_shared(cache->objects_qutex);
