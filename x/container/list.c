...
static void list_object_destroy(x_container_list_t *list,
    list_object_t *list_object);

static void remove_list_object(x_container_list_t *list,
    list_object_t *list_object);

static void update_first_and_last_on_remove(x_container_list_t *list,
    list_object_t *list_object);
    ...
    x_core_message_t *message, x_core_message_add_to_message_f add_to_message)
{
  ...
  list_size = x_container_list_get_size(list);
    ...
    void *object)
{
  ...
  if (list->size_limit) {
    if (list->size == list->size_limit) {
      x_container_list_remove_last(list);
    }
  }
    ...
    void *object)
{
  ...
  if (list->size_limit) {
    if (list->size == list->size_limit) {
      x_container_list_remove_first(list);
    }
  }
...
void x_container_list_clear(x_container_list_t *list)
{
  ...
  x_container_list_iterate_start(list);
  while (x_container_list_iterate_next(list)) {
...
int x_container_list_compare(void *list_object_a,
    void *list_object_b)
{
  assert(list_object_a);
  assert(list_object_b);
  x_container_list_t *list_a;
  x_container_list_t *list_b;
  int compare_result;
  void *object_a;
  void *object_b;
  x_core_compare_f compare;
  ...
  if (0 == compare_result) {
    ...
    x_container_list_iterate_start(list_a);
    x_container_list_iterate_start(list_b);
    while ((object_a = x_container_list_iterate_next(list_a))) {
      object_b = x_container_list_iterate_next(list_b);
...
void *x_container_list_copy(void *list_object)
{
  ...
  x_container_list_t *copy;
  void *original_object;
  void *copied_object;

  list = list_object;

  copy = x_container_list_create(list->compare, list->copy,
      list->destroy);
  if (copy) {
    x_container_list_iterate_start(list);
    while ((original_object = x_container_list_iterate_next(list))) {
      copied_object = list->copy(original_object);
      if (copied_object) {
        if (!x_container_list_add_last(copy, copied_object)) {
          x_core_trace("x_container_list_add_last");
          list->destroy(copied_object);
          x_container_list_destroy(copy);
          copy = NULL;
          break;
        }
      } else {
...
x_container_list_t *x_container_list_create
...
x_container_list_t *x_container_list_create_from_array_n(x_container_array_t *array,
    unsigned long max_size)
{
  ...
  compare = x_container_array_get_compare(array);
  copy = x_container_array_get_copy(array);
  destroy = x_container_array_get_destroy(array);
  array_size = x_container_array_get_size(array);

  list = x_container_list_create(compare, copy, destroy);
  if (list) {
    object_index = 0;
    while ((list->size < max_size) && (object_index < array_size)) {
      object = x_container_array_find(array, object_index);
      if (object) {
        object_copy = copy(object);
        if (object_copy) {
          if (!x_container_list_add_last(list, object_copy)) {
            x_core_trace("x_container_list_add_last");
          }
        } else {
          x_core_trace("copy");
    ...
    x_core_message_create_from_message_f create_from_message)
{
  ...
  if (list) {
    ...
    x_core_trace("x_container_list_create() failed");
  }
    ...
    const char *separators)
{
  ...
  if (list) {
    ...
    while (token) {
      ...
      if (token_copy) {
        if (!x_container_list_add_last(list, token_copy)) {
          x_core_trace("x_container_list_add_last");
...
void *x_container_list_find_at(x_container_list_t *list,
    unsigned long index)
{
  /*
    ...
    calls to x_container_list_find_at() efficient
  */
...
void *x_container_list_find_random(x_container_list_t *list)
{
  ...
  if (list->size > 0) {
    ...
    object = x_container_list_find_at(list, random_index);
  } else {
    object = NULL;
  }
...
void x_container_list_init_objectey(x_core_objectey_t *objectey)
{
  ...
  objectey->get_as_string = x_container_list_get_as_string;
  objectey->compare = x_container_list_compare;
  objectey->copy = x_container_list_copy;
  objectey->destroy = x_container_list_destroy;
}
...
void list_object_destroy(x_container_list_t *list, list_object_t *list_object)
{
  assert(list);
  assert(list_object);
...
void remove_list_object(x_container_list_t *list, list_object_t *list_object)
{
...
void update_first_and_last_on_remove(x_container_list_t *list,
    list_object_t *list_object)
{
  assert(list_object);
