...
int main(int argc, char *argv[])
{
  ...
  client = cf_x_net_client_create(server_ip, server_port, server_port,
      CF_X_NET_ENGINE_NO_GET_NAME_FUNCTION, &ping_client_context, log);
  if (!client) {
    cf_x_core_trace_exit("x_net_client_create");
  }

  if (!cf_x_net_client_register_engine(client, CF_X_NET_ENGINE_PING,
          cf_x_PINGPONG_MESSAGE_TYPE_COUNT)) {
    cf_x_core_trace_exit("x_net_client_register_engine");
  }

  cf_x_net_client_register_message_handler(client, CF_X_NET_ENGINE_PING,
