    ...
    cf_x_audit_log_t *log)
{
  ...
  if (system) {
    ...
    cf_sage_box_coordinate_init_witx_xyz(&dimension_coordinate,
        CF_SAGE_COR3_BOX_DIMENSION, CF_SAGE_COR3_BOX_DIMENSION, CF_SAGE_COR3_BOX_DIMENSION);
    cf_sage_cor3_actor_init_actorey(&system->actorey);
    system->box = cf_sage_core_create_actor_box(system, &dimension_coordinate,
        initial_solutions, &system->actorey, log);
    if (!system->box) {
      cf_x_audit_log_trace(log, "cor3", "sage_core_create_actor_box");
      free(system);
      system = NULL;
    }
  } else {
    ...
    unsigned short max_solution_count)
{
  ...
  solutions = cf_sage_core_create_solutions_from_box(system->box,
      max_solution_count, cf_sage_cor3_actor_get_solution, system->score_solution,
      system->goal, cf_sage_cor3_actor_compare_maximize,
      cf_sage_cor3_actor_compare_minimize, cf_sage_cor3_actor_copy, system->context,
      system->log);
  if (!solutions) {
...
{
  cf_sage_searcx_searchey_init(searchey, cf_sage_cor3_system_create,
      cf_sage_cor3_system_destroy, cf_sage_cor3_system_get_solutions_copy,
      cf_sage_cor3_system_search);
}
