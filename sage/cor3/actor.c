...
static void dance(cf_sage_cor3_actor_t *destination, cf_sage_cor3_actor_t *source_a,
    cf_sage_cor3_actor_t *source_b);

static cf_sage_cor3_actor_t *find_best_nearby_excluding_me(cf_sage_cor3_actor_t *actor);

static cf_sage_cor3_actor_t *find_worst_nearby_including_me(cf_sage_cor3_actor_t *actor);

static unsigned char get_element(cf_sage_cor3_actor_t *actor, unsigned short x,
    unsigned short y, unsigned short z);

static double get_score(cf_sage_cor3_actor_t *actor);
...
static void set_element(cf_sage_cor3_actor_t *actor, unsigned short x,
    unsigned short y, unsigned short z, unsigned char value);

void dance(cf_sage_cor3_actor_t *destination, cf_sage_cor3_actor_t *source_a,
    cf_sage_cor3_actor_t *source_b)
{
  assert(destination);
  assert(source_a);
...
cf_sage_cor3_actor_t *find_best_nearby_excluding_me(cf_sage_cor3_actor_t *actor)
{
  assert(actor);
  cf_sage_box_coordinate_t relative_coordinate;
  cf_sage_cor3_actor_t *neighbor_actor;
  cf_sage_cor3_actor_t *best_actor;
  double best_score;
  ...
  goal = cf_sage_cor3_system_get_goal(actor->system);
  box = cf_sage_cor3_system_get_box(actor->system);
  ...
  for (relative_coordinate.x = 0;
    ...
    for (relative_coordinate.y = 0;
      ...
      for (relative_coordinate.z = 0;
        ...
        neighbor_actor = cf_sage_box_system_find_relative(box, actor,
            &relative_coordinate);
...
cf_sage_cor3_actor_t *find_worst_nearby_including_me(cf_sage_cor3_actor_t *actor)
{
  assert(actor);
  cf_sage_box_coordinate_t relative_coordinate;
  cf_sage_cor3_actor_t *neighbor_actor;
  cf_sage_cor3_actor_t *worst_actor;
  double worst_score;
  double neighbor_score;
  cf_sage_core_goal_t goal;
  cf_sage_box_system_t *box;
...
unsigned char get_element(cf_sage_cor3_actor_t *actor, unsigned short x,
    unsigned short y, unsigned short z)
{
  assert(actor);
  assert(x <= 3);
...
double get_score(cf_sage_cor3_actor_t *actor)
{
  assert(actor);
  void *context;
  ...
  if (!actor->score_is_valid) {
    context = cf_sage_cor3_system_get_context(actor->system);
    score_solution = cf_sage_cor3_system_get_score_solution(actor->system);
    if (score_solution(context, actor->solution, &actor->score)) {
      actor->score_is_valid = cf_x_core_bool_true;
    } else {
      log = cf_sage_cor3_system_get_log(actor->system);
      cf_x_audit_log_trace(log, "cor3", "score_solution");
    }
  }
...
{
  assert(actor);
  cf_sage_cor3_actor_t *best_other_actor;
  cf_sage_cor3_actor_t *worst_actor;
  cf_sage_box_system_t *box;
  ...
  cf_sage_box_system_swap(box, actor, best_other_actor);
...
int cf_sage_cor3_actor_compare_maximize(void *actor_a_void, void *actor_b_void)
{
  assert(actor_a_void);
  assert(actor_b_void);
  cf_sage_cor3_actor_t **actor_a;
  cf_sage_cor3_actor_t **actor_b;
  double actor_a_score;
  double actor_b_score;
  int compare_result;

  actor_a = (cf_sage_cor3_actor_t **) actor_a_void;
  actor_b = (cf_sage_cor3_actor_t **) actor_b_void;

  actor_a_score = get_score((cf_sage_cor3_actor_t *) *actor_a);
  actor_b_score = get_score((cf_sage_cor3_actor_t *) *actor_b);
...
void *cf_sage_cor3_actor_create(void *system_void, cf_x_core_bitarray_t *solution)
{
  ...
  log = cf_sage_cor3_system_get_log(system);
  ...
  if (actor) {
    ...
    if (!actor->solution) {
      cf_x_audit_log_trace(log, "cor3", "x_core_bitarray_copy");
      free(actor);
      actor = NULL;
    }
  } else {
...
void *cf_sage_cor3_actor_create_random(void *system_void)
{
  ...
  if (bitarray) {
    actor = cf_sage_cor3_actor_create(system, bitarray);
    if (!actor) {
      log = cf_sage_cor3_system_get_log(system);
      cf_x_audit_log_trace(log, "cor3", "sage_cor3_actor_create");
    }
    cf_x_core_bitarray_destroy(bitarray);
  } else {
    actor = NULL;
    log = cf_sage_cor3_system_get_log(system);
    cf_x_audit_log_trace(log, "cor3", "x_core_bitarray_create_random");
  }
...
{
  cf_sage_core_actorey_init(actorey, cf_sage_cor3_actor_create,
      cf_sage_cor3_actor_create_random, cf_sage_cor3_actor_compare_minimize,
      cf_sage_cor3_actor_copy, cf_sage_cor3_actor_destroy, cf_sage_cor3_actor_get_box_cell,
      cf_sage_cor3_actor_set_box_cell);
}
...
void set_element(cf_sage_cor3_actor_t *actor, unsigned short x, unsigned short y,
