...
#include "cf/sage/ca/crow.h"
#include "cf/sage/ca/eca.h"
#include "cf/sage/ca/fly.h"
#include "cf/sage/ca/k3.h"
#include "cf/sage/ca/malice.h"
...
{
  cf_sage_ca_system_t *cell_system;
  cf_x_case_array_t *initial_state_history;
  unsigned long initial_time_step_count;
  unsigned long cell_count;
  cf_x_core_run_t *run;
  char *filename;
  cf_sage_ca_systemey_t cell_systemey;

  cell_count = 256;
  initial_time_step_count = 256;
  initial_state_history
    = cf_sage_ca_create_initial_state_single_cell_binary(cell_count, 2);

  cf_sage_ca_ant_init_systemey(&cell_systemey, name);

  cell_system = cf_sage_ca_system_create(initial_state_history,
      initial_time_step_count, &cell_systemey);
  if (!cell_system) {
    cf_x_core_trace("sage_ca_system_create");
  }
  ...
  cf_sage_ca_system_run(cell_system, run);
  ...
  if (!cf_sage_ca_system_save_snapshot_jpeg(cell_system, filename)) {
    cf_x_core_trace("sage_ca_system_save_snapshot_jpeg");
  }

  free(filename);

  cf_sage_ca_system_destroy(cell_system);
...
void test_clear(unsigned short name)
{
  ...
  initial_time_step_count = 256;
  initial_state_history
    = cf_sage_ca_create_initial_state_single_cell_binary(cell_count, 1);

  cf_sage_ca_clear_init_systemey(&cell_systemey, &name);
...
void test_crow(unsigned short name)
{
  ...
  cf_sage_ca_crow_init_systemey(&cell_systemey, &name);
...
void test_eca(unsigned short name)
{
  ...
  cf_sage_ca_eca_init_systemey(&cell_systemey, &name);
...
void test_fly(char *name)
{
  ...
  cf_sage_ca_fly_init_systemey(&cell_systemey, name);
...
void test_k3(unsigned long name)
{
  ...
  initial_time_step_count = 128;
  initial_state_history
    = cf_sage_ca_create_initial_state_single_cell_k3(cell_count);

  cf_sage_ca_k3_init_systemey(&cell_systemey, &name);
...
void test_malice(char *name)
{
  ...
  cf_sage_ca_malice_init_systemey(&cell_systemey, name);
...
void test_water(char *name)
{
  ...
  initial_state_history
    = cf_sage_ca_create_initial_state_single_cell_binary(cell_count, 3);

  cf_sage_ca_water_init_systemey(&cell_systemey, name);
