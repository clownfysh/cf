...
void *cf_sage_ca_state_copy(void *state_object)
{
  ...
  state = state_object;
  cells_memory_size = sizeof(cf_sage_ca_t) * state->cell_count;
...
cf_sage_ca_state_t *cf_sage_ca_state_create(unsigned long cell_count)
{
  ...
  if (state) {
    ...
    state->cells = malloc(sizeof(cf_sage_ca_t) * cell_count);
    if (!state->cells) {
      cf_x_core_trace("malloc");
    }
  } else {
    ...
    unsigned long cell_count)
{
  ...
  cells_memory_size = sizeof(cf_sage_ca_state_t) * cell_count;
...
char *cf_sage_ca_state_get_as_string(void *state_object)
{
  ...
  char c;
  cf_sage_ca_t *cell;
  unsigned long cf_sage_ca_value;
  unsigned long cell_count;
  ...
  if (string) {
    for (each_cell = 0; each_cell < cell_count; each_cell++) {
      ...
      cf_sage_ca_value = cell->value;
      if (cf_sage_ca_value < 10) {
        c = 48 + cf_sage_ca_value;
      } else {
        c = 120;
      }
      *(string + each_cell) = c;
