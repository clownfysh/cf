...
static cf_x_core_bool_t init_box_add_random_actors(cf_sage_box_system_t *box,
    cf_sage_box_coordinate_t *dimension_coordinate,
    cf_sage_core_create_actor_random_f create_actor_random, void *searcx_system,
    cf_x_audit_log_t *log);

static cf_x_core_bool_t init_box_add_actors_from_initial_solutions
(cf_sage_box_system_t *box, cf_sage_core_actorey_create_f actor_create,
    ...
    cf_x_core_copy_f copy_object, void *context, cf_x_audit_log_t *log)
{
  ...
  box_volume = cf_sage_box_system_get_volume(box);
  ...
  if (objects) {
    cf_sage_box_system_iterate_start(box);
    index = 0;
    while ((object = cf_sage_box_system_iterate_next(box))) {
    ...
    cf_x_audit_log_t *log)
{
  ...
  box = cf_sage_box_system_create(dimension_coordinate, actorey->get_box_cell,
  ...
  if (box) {
    if (init_box_add_random_actors(box, dimension_coordinate,
      ...
      if (initial_solutions) {
        if (!init_box_add_actors_from_initial_solutions(box, actorey->create,
          ...
          cf_sage_box_system_destroy(box);
          box = NULL;
    ...
    cf_x_audit_log_trace(log, "ih", "sage_box_system_create");
  }

  return box;
}

cf_x_core_bool_t init_box_add_random_actors(cf_sage_box_system_t *box,
    cf_sage_box_coordinate_t *dimension_coordinate,
    cf_sage_core_create_actor_random_f create_actor_random, void *searcx_system,
    cf_x_audit_log_t *log)
{
  ...
  for (coordinate.x = 0;
    ...
    for (coordinate.y = 0;
      ...
      for (coordinate.z = 0;
        ...
        if (actor) {
          cf_sage_box_system_add(box, &coordinate, actor);
        } else {
...
cf_x_core_bool_t init_box_add_actors_from_initial_solutions(cf_sage_box_system_t *box,
    cf_sage_core_actorey_create_f actor_create, void *searcx_system,
    cf_x_case_array_t *initial_solutions, cf_x_audit_log_t *log)
{
  ...
  while ((bitarray = cf_x_case_array_iterate_next(initial_solutions))) {
    ...
    if (actor) {
      cf_sage_box_system_replace_random(box, actor);
    } else {
      success = cf_x_core_bool_false;
    ...
    unsigned long gene_index)
{
  ...
  address_of_gene_header = CF_SAGE_CORE_SOLUTION_BIT_COUNT_2N * gene_index;
  start_address = cf_x_core_bitarray_get_unsigned_short(solution,
      address_of_gene_header) % CF_SAGE_CORE_SOLUTION_BIT_COUNT;
