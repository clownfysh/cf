...
struct cf_sage_computer_system_t {
  ...
  cf_sage_computer_output_t *computations;
};

static void compute_witx_logic(cf_sage_computer_system_t *system);

static void free_computations(cf_sage_computer_system_t *system);

static void init_computations(cf_sage_computer_system_t *system);

void compute_witx_logic(cf_sage_computer_system_t *system)
{
  assert(system);
...
cf_sage_computer_system_t *cf_sage_computer_system_create(unsigned long order,
    cf_sage_computer_logic_f logic)
{
  cf_sage_computer_system_t *system;

  system = malloc(sizeof *system);
  if (system) {
    system->order = order;
    system->logic = logic;
    system->computations = malloc(order * sizeof(cf_sage_computer_output_t));
...
char *cf_sage_computer_system_get_as_string(void *system_object)
{
  ...
  string_size = (CF_SAGE_COMPUTER_OUTPUT_BITS + 1) * system->order;
  string = malloc(string_size + 1);
  if (string) {
    string_position = 0;
    for (each_computation = 0; each_computation < system->order;
         each_computation++) {
      for (each_bit = 0; each_bit < CF_SAGE_COMPUTER_OUTPUT_BITS; each_bit++) {
...
void cf_sage_computer_system_print(cf_sage_computer_system_t *system)
{
  ...
  string = cf_sage_computer_system_get_as_string(system);
  if (string) {
    printf("%s", string);
    free(string);
  } else {
    cf_x_core_trace("sage_computer_system_get_as_string");
  }
}
...
void free_computations(cf_sage_computer_system_t *system)
{
  unsigned long each_computation;

  for (each_computation = 0; each_computation < system->order;
       each_computation++) {
    cf_sage_computer_output_free(system->computations + each_computation);
  }
}

void init_computations(cf_sage_computer_system_t *system)
{
  unsigned long each_computation;

  for (each_computation = 0; each_computation < system->order;
       each_computation++) {
    cf_sage_computer_output_init(system->computations + each_computation);
  }
}
