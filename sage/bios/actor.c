...
char display_color(cf_sage_bios_actor_t *actor);

static unsigned long eight_from_eight(cf_sage_bios_actor_t *actor,
...
static unsigned long eight_from_sixteen(cf_sage_bios_actor_t *actor,
    unsigned long ulong_0, unsigned long ulong_1, unsigned long ulong_2,
    unsigned long ulong_3, unsigned long ulong_4, unsigned long ulong_5,
...
static unsigned long eight_from_twenty_four(cf_sage_bios_actor_t *actor,
    unsigned long ulong_0, unsigned long ulong_1, unsigned long ulong_2,
    unsigned long ulong_3, unsigned long ulong_4, unsigned long ulong_5,
    unsigned long ulong_6, unsigned long ulong_7, unsigned long ulong_8,
    unsigned long ulong_9, unsigned long ulong_10, unsigned long ulong_11,
...
(cf_sage_bios_actor_t *actor, unsigned long observe_location);

static cf_sage_box_coordinate_t get_relative_position_from_index
(unsigned long index);

static double get_score(cf_sage_bios_actor_t *actor);

static void meet_actor(cf_sage_bios_actor_t *actor);

static void meet_actor_details(cf_sage_bios_actor_t *actor_a,
    cf_sage_bios_actor_t *actor_b);

static void move_actor(cf_sage_bios_actor_t *actor);

static void parse_display_gene(cf_sage_bios_actor_t *actor,
    unsigned long gene_start_address, display_gene_t *display_gene);

static void parse_meet_gene(cf_sage_bios_actor_t *actor,
...
unsigned long eight_from_eight(cf_sage_bios_actor_t *actor, unsigned long ulong_0,
...
unsigned long eight_from_sixteen(cf_sage_bios_actor_t *actor, unsigned long ulong_0,
    unsigned long ulong_1, unsigned long ulong_2, unsigned long ulong_3,
    unsigned long ulong_4, unsigned long ulong_5, unsigned long ulong_6,
...
unsigned long eight_from_twenty_four(cf_sage_bios_actor_t *actor,
    unsigned long ulong_0, unsigned long ulong_1, unsigned long ulong_2,
    unsigned long ulong_3, unsigned long ulong_4, unsigned long ulong_5,
    unsigned long ulong_6, unsigned long ulong_7, unsigned long ulong_8,
    unsigned long ulong_9, unsigned long ulong_10, unsigned long ulong_11,
...
unsigned long get_position_index_for_observe_location(cf_sage_bios_actor_t *actor,
    unsigned long observe_location)
{
  ...
  cf_sage_bios_actor_t *actor_0;
  cf_sage_bios_actor_t *actor_1;
  cf_sage_bios_actor_t *actor_2;
  cf_sage_bios_actor_t *actor_3;
  cf_sage_bios_actor_t *actor_4;
  cf_sage_bios_actor_t *actor_5;
  cf_sage_bios_actor_t *actor_6;
  cf_sage_bios_actor_t *actor_7;

  cf_sage_box_coordinate_t relation;
  unsigned long new_position_index;
  cf_sage_box_system_t *box;

  box = cf_sage_bios_system_get_box(actor->system);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 0, 0, 1);
  actor_0 = cf_sage_box_system_find_relative(box, actor, &relation);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 1, 0, 1);
  actor_1 = cf_sage_box_system_find_relative(box, actor, &relation);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 2, 0, 1);
  actor_2 = cf_sage_box_system_find_relative(box, actor, &relation);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 2, 1, 1);
  actor_3 = cf_sage_box_system_find_relative(box, actor, &relation);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 2, 2, 1);
  actor_4 = cf_sage_box_system_find_relative(box, actor, &relation);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 1, 2, 1);
  actor_5 = cf_sage_box_system_find_relative(box, actor, &relation);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 0, 2, 1);
  actor_6 = cf_sage_box_system_find_relative(box, actor, &relation);

  cf_sage_box_coordinate_init_witx_xyz(&relation, 0, 1, 1);
  actor_7 = cf_sage_box_system_find_relative(box, actor, &relation);
...
cf_sage_box_coordinate_t get_relative_position_from_index(unsigned long index)
{
  assert(index < 8);
  cf_sage_box_coordinate_t position;
...
double get_score(cf_sage_bios_actor_t *actor)
{
  assert(actor);
  ...
  if (!actor->score_is_valid) {
    context = cf_sage_bios_system_get_context(actor->system);
    score_solution = cf_sage_bios_system_get_score_solution(actor->system);
    if (score_solution(context, actor->solution, &actor->score)) {
      actor->score_is_valid = cf_x_core_bool_true;
    } else {
      log = cf_sage_bios_system_get_log(actor->system);
...
void *cf_sage_bios_actor_create(void *system_void, cf_x_core_bitarray_t *solution)
{
  ...
  if (actor) {
    ...
    if (!actor->solution) {
      cf_x_core_trace("sage_core_birarray_copy");
      free(actor);
      actor = NULL;
    }
  } else {
...
void *cf_sage_bios_actor_create_random(void *system_void)
{
  ...
  if (bitarray) {
    actor = cf_sage_bios_actor_create(system, bitarray);
    if (!actor) {
      log = cf_sage_bios_system_get_log(system);
      cf_x_audit_log_trace(log, "bios", "sage_bios_actor_create");
    }
...
int cf_sage_bios_actor_compare_maximize(void *actor_a_void, void *actor_b_void)
{
  assert(actor_a_void);
  assert(actor_b_void);
  cf_sage_bios_actor_t **actor_a;
  cf_sage_bios_actor_t **actor_b;
  double actor_a_score;
  double actor_b_score;
  long compare_result;

  actor_a = (cf_sage_bios_actor_t **) actor_a_void;
  actor_b = (cf_sage_bios_actor_t **) actor_b_void;

  actor_a_score = get_score((cf_sage_bios_actor_t *) *actor_a);
  actor_b_score = get_score((cf_sage_bios_actor_t *) *actor_b);
...
void cf_sage_bios_actor_init_actorey(cf_sage_core_actorey_t *actorey)
{
  cf_sage_core_actorey_init(actorey,
      cf_sage_bios_actor_create, cf_sage_bios_actor_create_random,
      cf_sage_bios_actor_compare_minimize, cf_sage_bios_actor_copy,
      cf_sage_bios_actor_destroy, cf_sage_bios_actor_get_box_cell,
      cf_sage_bios_actor_set_box_cell);
}
...
void meet_actor(cf_sage_bios_actor_t *actor)
{
  unsigned long exchange_position_index;
  cf_sage_box_coordinate_t exchange_position_relative;
  cf_sage_bios_actor_t *actor_to_exchange_with;
...
void meet_actor_details(cf_sage_bios_actor_t *actor_a, cf_sage_bios_actor_t *actor_b)
{
  assert(actor_a);
  assert(actor_b);
  ...
  box = cf_sage_bios_system_get_box(actor_a->system);
  cf_sage_box_system_swap(box, actor_a, actor_b);
}

void move_actor(cf_sage_bios_actor_t *actor)
{
  assert(actor);
  unsigned long new_position_index;
  cf_sage_box_coordinate_t new_position_relative;
  cf_sage_box_system_t *box;
  cf_sage_bios_actor_t *actor_to_switcsage_with;
  ...
  actor_to_switcsage_with = cf_sage_box_system_find_relative(box, actor,
      &new_position_relative);
  cf_sage_box_system_swap(box, actor, actor_to_switcsage_with);
}

void parse_display_gene(cf_sage_bios_actor_t *actor,
    unsigned long gene_start_address, display_gene_t *display_gene)
{
...
*/
void parse_meet_gene(cf_sage_bios_actor_t *actor, unsigned long gene_start_address,
    meet_gene_t *meet_gene)
{
