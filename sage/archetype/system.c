...
(cf_sage_archetype_system_t *system);

cf_x_core_bool_t calculate_poles_and_weights(cf_sage_archetype_system_t *system)
{
  assert(system);
  ...
  for (attribute_index = 0; attribute_index < system->attribute_count;
    ...
    if (weight_copy) {
      ...
      success = cf_x_core_bool_false;
      cf_x_audit_log_trace(system->log, "arch", "sage_core_double_copy");
    }
  }
    ...
    cf_x_audit_log_t *log)
{
  ...
  if (system) {
    ...
    if (system->classified_objects) {
      ...
      cf_x_audit_log_trace(log, "arch", "x_case_array_copy");
    } else {
  ...
  if (so_far_so_good) {
    if (!cf_sage_archetype_system_learn(system)) {
      so_far_so_good = cf_x_core_bool_false;
      cf_x_audit_log_trace(log, "arch", "sage_archetype_system_learn");
    }
  }
    ...
    cf_x_core_bitarray_t *object)
{
  ...
  assert(cf_x_core_bitarray_get_size(object)
      >= ((cf_sage_archetype_system_t *) system_void)->attribute_count);
  cf_sage_archetype_system_t *system;
  cf_x_core_bit_t class;
  unsigned long attribute_index;
...
{
  return cf_sage_classify_classifyey_init(classifyey, cf_sage_archetype_system_create,
      cf_sage_archetype_system_destroy, cf_sage_archetype_system_classify_object,
      cf_sage_archetype_system_learn, cf_sage_archetype_system_observe_object);
}
...
cf_x_core_bool_t cf_sage_archetype_system_learn(void *system_void)
{
  ...
  if (system->new_objects_since_last_learn) {
    if (!calculate_poles_and_weights(system)) {
      success = cf_x_core_bool_false;
      cf_x_audit_log_trace(system->log, "arch", "calculate_poles");
    }
    if (success) {
    ...
    cf_x_core_bitarray_t *classified_object)
{
  ...
  if (classified_object_copy) {
    ...
    success = cf_x_core_bool_false;
    cf_x_audit_log_trace(system->log, "arch", "x_core_bitarray_copy");
  }
