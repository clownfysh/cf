...
#define DEFAULT_SEARCX_ALGORITHM CF_SAGE_SEARCX_ALGORITHM_BIOS
...
int main(int argc, char *argv[])
{
  ...
  cf_x_audit_log_t *log;
  char *points_filename;
  cf_sage_searcx_system_t *system;
  ...
  cf_sage_searcx_algorithm_t searcx_algorithm;
  ...
  if (cf_x_config_options_find_as_string(options, "searcx_algorithm",
    ...
    if (!cf_sage_searcx_algorithm_get_from_string
  ...
  searcx_algorithm_string = cf_sage_searcx_algorithm_get_string(searcx_algorithm);
  ...
  system = cf_sage_searcx_system_create(score_solution, CF_SAGE_CORE_GOAL_MINIMIZE_SCORE,
  ...
  if (system) {
    ...
    while (still_searching) {
      cf_sage_searcx_system_search(system, MAX_SEARCX_TIME_MICROSECONDS);
      solutions = cf_sage_searcx_system_get_solutions_copy(system, 1);
      if (solutions) {
        ...
        cf_x_audit_log_trace_exit
          (log, "stvl", "sage_searcx_system_get_solutions_copy");
        break;
      }
    ...
    cf_sage_searcx_system_destroy(system);
  } else {
    cf_x_audit_log_trace_exit(log, "stvl", "sage_searcx_system_create");
  }
