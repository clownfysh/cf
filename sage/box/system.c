...
struct cf_sage_box_cell_t;
typedef struct cf_sage_box_cell_t cf_sage_box_cell_t;

struct cf_sage_box_cell_t {
  cf_sage_box_system_t *system;
  cf_sage_box_coordinate_t coordinate;
  cf_sage_box_cell_t *neighbors[3][3][3];
  void *object;
};

#ifdef CF_BUILD_DEVELOPMENT
static cf_x_core_bool_t coordinate_in_range(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate);
#endif

static cf_x_core_bool_t create_cells(cf_sage_box_system_t *system);

static cf_sage_box_cell_t *find_cell(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate);

static unsigned long get_cells_array_index(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate);

static int cf_sage_box_cell_compare(void *cell_a_void, void *cell_b_void);

static void *cf_sage_box_cell_copy(void *cell_void);

static cf_sage_box_cell_t *cf_sage_box_cell_create(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate);

static void cf_sage_box_cell_destroy(void *cell_void);
...
static void set_neighbor_references(cf_sage_box_system_t *system);

static void set_neighbor_references_for_cell(cf_sage_box_system_t *system,
    cf_sage_box_cell_t *cell);

#ifdef CF_BUILD_DEVELOPMENT
cf_x_core_bool_t coordinate_in_range(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate)
{
  assert(system);
  assert(coordinate);
...
cf_x_core_bool_t create_cells(cf_sage_box_system_t *system)
{
  assert(system);
  ...
  system->cells = cf_x_case_array_create(system->volume,
      cf_sage_box_cell_compare, cf_sage_box_cell_copy, cf_sage_box_cell_destroy);
  if (system->cells) {
...
cf_sage_box_cell_t *find_cell(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate)
{
  assert(system);
  assert(coordinate);
...
unsigned long get_cells_array_index(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate)
{
...
int cf_sage_box_cell_compare(void *cell_a_void, void *cell_b_void)
{
  cf_x_core_trace_exit("TODO: implement");
  return 0;
}

void *cf_sage_box_cell_copy(void *cell_void)
{
  cf_x_core_trace_exit("TODO: implement");
  return NULL;
}

cf_sage_box_cell_t *cf_sage_box_cell_create(cf_sage_box_system_t *system,
    cf_sage_box_coordinate_t *coordinate)
{
  assert(system);
  assert(coordinate);
  ...
  if (cell) {
    cell->system = system;
    cf_sage_box_coordinate_init_witx_coordinate(&cell->coordinate, coordinate);
    cell->object = NULL;
...
void cf_sage_box_cell_destroy(void *cell_void)
{
  assert(cell_void);
  cf_sage_box_cell_t *cell;
  cf_sage_box_system_t *system;
...
void cf_sage_box_system_add_random(cf_sage_box_system_t *system, void *object)
{
  ...
  cf_sage_box_coordinate_init_witx_random(&coordinate,
      &system->dimension_coordinate);
  cf_sage_box_system_add(system, &coordinate, object);
}

cf_sage_box_system_t *cf_sage_box_system_create
(cf_sage_box_coordinate_t *dimension_coordinate,
    cf_sage_box_object_get_cell_f get_cell, cf_sage_box_object_set_cell_f set_cell,
    ...
    cf_x_core_destroy_f destroy_object, cf_x_audit_log_t *log)
{
  ...
  if (system) {
    cf_sage_box_coordinate_copy(&system->dimension_coordinate,
...
void *cf_sage_box_system_find_relative(cf_sage_box_system_t *system,
    void *object, cf_sage_box_coordinate_t *relative_coordinate)
{
  assert(system);
  assert(object);
    ...
    cf_sage_box_coordinate_t *destination_coordinate)
{
  ...
  cf_sage_box_cell_t *source_cell;
  cf_sage_box_cell_t *destination_cell;
  ...
  if (source_cell != destination_cell) {
    cf_sage_box_system_remove(system, destination_coordinate);
    cf_sage_box_system_add(system, destination_coordinate, object);
    ...
    cf_sage_box_coordinate_t *relative_coordinate)
{
  ...
  if (source_cell != destination_cell) {
    cf_sage_box_system_remove(system, &destination_cell->coordinate);
    cf_sage_box_system_add(system, &destination_cell->coordinate, object);
    source_cell->object = NULL;
  }
}
    ...
    void *source_object)
{
  ...
  cf_sage_box_system_remove(system, &destination_cell->coordinate);
  cf_sage_box_system_add(system, &destination_cell->coordinate, source_object);
}
...
void cf_sage_box_system_replace_random(cf_sage_box_system_t *system, void *object)
{
  ...
  cf_sage_box_system_remove(system, &coordinate);
  cf_sage_box_system_add(system, &coordinate, object);
}
    ...
    void *object_b)
{
  ...
  assert(object_a);
  assert(object_b);
  cf_sage_box_cell_t *cell_a;
  cf_sage_box_cell_t *cell_b;
...
void set_neighbor_references(cf_sage_box_system_t *system)
{
  assert(system);
  cf_sage_box_cell_t *cell;
...
void set_neighbor_references_for_cell(cf_sage_box_system_t *system,
    cf_sage_box_cell_t *cell)
{
  assert(system);
  assert(cell);
  cf_sage_box_coordinate_t neighbor_coordinate;
  cf_sage_box_coordinate_t relative_coordinate;
  cf_sage_box_cell_t *neighbor;
